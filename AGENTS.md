...
External Notion/Google export is Flutter UI plus Edge Functions (`integrations`, `integrations-oauth-callback`). Flutter may read connection metadata only. After protocol v2 generation advances, Flutter waits 1 minute then one-way exports connected providers that are behind; do not hook export to every `SyncService.syncNow()` or the 60-second poll. Tokens stay in the private `integrations` schema; do not expose that schema on the Data API. `INTEGRATION_TOKEN_KEY` is a non-empty UTF-8 string; the function SHA-256s it for AES-GCM. Do not put OAuth secrets in `.env`, git or chat. CLI/MCP do not implement provider OAuth. See `docs/external-integrations.md`. The additive migration is `20260908120000_add_external_integrations.sql`; it must not rewrite protocol v2 tables. Supabase CLI migration filenames in this repo use unique 14-digit versions.
