...
External Notion/Google export is Flutter UI plus Edge Functions (`integrations`, `integrations-oauth-callback`). Flutter may read connection metadata, `external_sync_links` inbound_state and `external_sync_conflict_summaries` (never conflict snapshots). Conflict resolve uses authenticated `integrations` actions `list_conflicts` / `resolve_conflict`. After protocol v2 generation advances, Flutter waits 1 minute then one-way exports connected providers that are behind; do not hook export to every `SyncService.syncNow()` or the 60-second poll. Do not poll Notion/Google from Flutter. Inbound is thin webhooks (`integrations-notion-webhook`, `integrations-google-webhook`) plus `integrations-inbound-worker`; linked updates go through `integrations.apply_external_change` and remote create/recovery through `integrations.reconcile_external_object`, never PostgREST business-table updates or `diurna_sync_*_v2`. Tokens, verification tokens, Google channel tokens, ciphertext and `INTEGRATIONS_MAINTENANCE_SECRET` stay in the private `integrations` schema; do not expose that schema on the Data API. `INTEGRATION_TOKEN_KEY` is a non-empty UTF-8 string; the function SHA-256s it for AES-GCM. Do not put OAuth or maintenance secrets in `.env`, git or chat. CLI/MCP do not implement provider OAuth. See `docs/external-integrations.md` and `docs/external-bidirectional-sync.md`. Export migration: `20260908120000_add_external_integrations.sql`. Hosted inbound/conflict migrations `20260909120000`–`20260909190000` are applied and immutable. Remote-create migration `20260910000000_external_remote_create.sql` is in the repository and not yet hosted. They must not rewrite protocol v2 tables. Once a migration has been applied hosted, never edit it; add a new additive file. Supabase CLI migration filenames in this repo use unique 14-digit versions. After a Notion import/export mapping change, redeploy both `integrations` (`verify_jwt=true`) and `integrations-inbound-worker` (`verify_jwt=false`). After inbound webhook event-set changes, also redeploy `integrations-notion-webhook` / `integrations-google-webhook` (`verify_jwt=false`).
...
For Supabase changes, add a migration and update `supabase/schema.sql` for clean installs. Do not re-run the `20260711000001` / `20260711000002` destructive migrations on existing user data. Follow `docs/sync-protocol.md` rollout order. Activating the v2 guard deliberately blocks old-client uploads; their local queue must survive until upgrade. Never edit a migration that has already been applied to the hosted project; create a new additive migration instead.
